const bands = [
  {
    bandName: 'Radiohead',
    bestAlbuns: [
      {
        name: "In Rainbows",
        rate: 100
      },
      {
        name: "Kid A",
        rate: 95
      },
      {
        name: "OK Computer",
        rate: 90
      },
      {
        name: "Pablo Honey",
        rate: 80
      }
    ],
    genre: "Rock",
    country: "England",
  },
  {
    bandName: 'Avenged Sevenfold',
    bestAlbuns: [
      {
        name: "City of Evil",
        rate: 99
      },
      {
        name: "Avenged Sevenfold",
        rate: 98
      },
      {
        name: "Sounding the Seventh Trumpet",
        rate: 95
      },
      {
        name: "Diamonds in the Rough",
        rate: 80
      }
    ],
    genre: 'Hard Rock',
    country: "United States",
  },
  {
    bandName: 'Arctic Monkeys',
    bestAlbuns: [
      {
        name: "Humbug",
        rate: 100
      },
      {
        name: "AM",
        rate: 99
      },
      {
        name: "Favourite Worst Nightmare",
        rate: 95
      },
      {
        name: "Whatever People Say I Am, That's What I'm Not",
        rate: 90
      }
    ],
    genre: 'Indie Rock',
    country: "England",
  },
  {
    bandName: 'Pink Floyd',
    bestAlbuns: [
      {
        name: "Wish You Were Here",
        rate: 100
      },
      {
        name: "Dark Side of the Moon",
        rate: 100
      },
      {
        name: "Animals",
        rate: 100
      },
      {
        name: "A Saucerful of Secrets",
        rate: 98
      }
    ],
    genre: 'Progressive Rock',
    country: "England",
  },
  {
    bandName: 'Foo Fighters',
    bestAlbuns: [
      {
        name: "Wasting Light",
        rate: 100
      },
      {
        name: "Sonic Highways",
        rate: 90
      },
      {
        name: "Concrete and Gold",
        rate: 90
      },
      {
        name: "Echoes, Silence, Patience & Grace",
        rate: 85
      }
    ],
    genre: 'Rock',
    country: "United States",
  },
  {
    bandName: 'Red Hot Chili Peppers',
    bestAlbuns: [
      {
        name: "Stadium Arcadium",
        rate: 100
      },
      {
        name: "Californication",
        rate: 99
      },
      {
        name: "By the Way",
        rate: 97
      },
      {
        name: "The Getaway",
        rate: 95
      }
    ],
    genre: 'Rock',
    country: "United States"
  }
]

const [ Radiohead = bandName ] = bands; // Destructuring de uma array para pegar o primeiro elemento da array, caso eu faça outro Destructuring, ele pega o proximo elemento
// console.log(Radiohead)

const bandsNames = bands.map(band => [band.bandName, band.genre]);

// console.log(bandsNames)

const usaBands = bands.filter(band => band.country === 'United States').map(band => band.bandName);

console.log(usaBands)